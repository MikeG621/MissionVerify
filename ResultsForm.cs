 ...
 * v2.3, 250106
 * [UPD YOGEME/#115] Fixes regarding Idmr.Platform v7
 * v2.2.1, 231223
 ...
 * v2.1, 210315
 * [ADD YOGEME/#48] Trigger "OR true" and "AND false" detection. NOTE: checking 1ao2 and 3ao4, but not 12ao34
 * v2.0, 180215
...
namespace Idmr.MissionVerify
{
	public partial class ResultsForm : Form
	{
		public ResultsForm(string file)
		{
			InitializeComponent();
			FileStream fsFile;
			try  { fsFile = File.Open(file, FileMode.Open, FileAccess.Read); }
			catch
			{
		...
		private void cmdOK_Click(object sender, System.EventArgs e) => Dispose();
		...
		private string[] tie(FileStream fsFile)
		{
			...
			if (mission.FlightGroups.Count > Platform.Tie.Mission.FlightGroupLimit)
				results.Add($"**Warning, mission has more than {Platform.Tie.Mission.FlightGroupLimit} Flight Groups, mission is NOT flyable.");
			...
			if (mission.Messages.Count > Platform.Tie.Mission.MessageLimit)
				results.Add($"**Mission has more than {Platform.Tie.Mission.MessageLimit} in-flight messages, errors will occur.");
			...
			if (craftCount > Platform.Tie.Mission.CraftLimit)
				results.Add($"*More than {Platform.Tie.Mission.CraftLimit} craft, ensure not all exist concurrently.");
			duplicate = false;
			foreach (Platform.Tie.FlightGroup fg in mission.FlightGroups)
			{
				...
				if (isBadTrigger(fg.ArrDepTriggers[0], fg.ArrDepTriggers[1], fg.AT1OrAT2))
			...
			for (int i = 0; i < mission.Messages.Count; i++)
				if (isBadTrigger(mission.Messages[i].Triggers[0], mission.Messages[i].Triggers[1], mission.Messages[i].Trig1OrTrig2))
			...
			for (int i = 0; i < 3; i++)
				if (isBadTrigger(mission.GlobalGoals.Goals[i].Triggers[0], mission.GlobalGoals.Goals[i].Triggers[1], mission.GlobalGoals.Goals[i].T1AndOrT2))
					results.Add($"**{(i == 0 ? "Primary" : (i == 1 ? "Secondary" : "Bonus"))} Global goals have trigger errors.");

			if (mission.Briefing.Length == 9998 || mission.Briefing.Length == 0x21C)
				results.Add("*Briefing has default length.");
			if (mission.Briefing.Events.Count < 2 || mission.Briefing.Events[0].Time == 9999 || mission.Briefing.Events[1].Time == 9999)
			...
			for (int i = 0; i < strRes.Length; i++) strRes[i] = results[i];
			return strRes;
		}
		...
		private string[] xvt(FileStream fsFile)
		{
			...
			if (mission.FlightGroups.Count > Platform.Xvt.Mission.FlightGroupLimit)
				results.Add($"**Warning, more than {Platform.Xvt.Mission.FlightGroupLimit} Flight Groups, mission may not be stable.");
			if (mission.Messages.Count == 0) 
				results.Add("Mission has no in-flight messages.");
			if (mission.Messages.Count > Platform.Xvt.Mission.MessageLimit)
				results.Add($"**Mission has more than {Platform.Xvt.Mission.MessageLimit} in-flight messages, errors will occur.");
			...
			if (craftCount > Platform.Xvt.Mission.CraftLimit)
				results.Add($"*More than {Platform.Xvt.Mission.CraftLimit} craft in mission, ensure all do not exist concurrently.");
			duplicate = false;
			foreach (Platform.Xvt.FlightGroup fg in mission.FlightGroups)
			{
				...
				if (isBadTrigger(fg.SkipToOrder4Trigger[0], fg.SkipToOrder4Trigger[1], fg.SkipToO4T1OrT2))
					results.Add($"*Flight Group {fg.Name} has Skip to Order trigger errors.");
			}

			for (int i = 0; i < mission.Messages.Count; i++)
				if (isBadTrigger(mission.Messages[i].Triggers[0], mission.Messages[i].Triggers[1], mission.Messages[i].T1OrT2) ||
					isBadTrigger(mission.Messages[i].Triggers[2], mission.Messages[i].Triggers[3], mission.Messages[i].T3OrT4))
					results.Add($"*Message {i} has trigger errors.");

			for (int t = 0; t < 10; t++)
				for (int g = 0; g < 3; g++)
					if (isBadTrigger(mission.Globals[t].Goals[g].Triggers[0].GoalTrigger, mission.Globals[t].Goals[g].Triggers[1].GoalTrigger, mission.Globals[t].Goals[g].T1OrT2) ||
						isBadTrigger(mission.Globals[t].Goals[g].Triggers[2].GoalTrigger, mission.Globals[t].Goals[g].Triggers[3].GoalTrigger, mission.Globals[t].Goals[g].T3OrT4))
						results.Add($"*Team {t + 1} {(g == 0 ? "Primary" : (g == 1 ? "Prevent" : "Secondary"))} Global goals have trigger errors.");
			if (mission.Briefings[0].Events.Count < 2 || mission.Briefings[0].Events[0].Time == 9999 || mission.Briefings[0].Events[1].Time == 9999)
				results.Add("*No briefing.");
		...
		private string[] xwa(FileStream fsFile)
		{
			...
			if (mission.FlightGroups.Count > Platform.Xwa.Mission.FlightGroupLimit)
				results.Add($"**Warning, over {Platform.Xwa.Mission.FlightGroupLimit} Flight Groups, mission may be unstable.");
			...
			if (mission.Messages.Count > Platform.Xwa.Mission.MessageLimit)
				results.Add($"**Warning, over {Platform.Xwa.Mission.MessageLimit} in-flight messages, mission may be unstable.");
			...
			{
				if (fg.CraftType != 0xB7 && (fg.CraftType < 0x46 || fg.CraftType > 0x58))
					craftCount += fg.NumberOfCraft;
				if (fg.PlayerNumber != 0)
				{
					results.Add((duplicate ? "*" : "") + $"Flight Group {fg.Name} is player craft.");
			...
			if (craftCount > Platform.Xwa.Mission.CraftLimit)
				results.Add($"*More than {Platform.Xwa.Mission.CraftLimit} craft. Ensure not all exist in the same region concurrently.");
			duplicate = false;
			foreach (Platform.Xwa.FlightGroup fg in mission.FlightGroups)
			{
				...
				if ((fg.CraftType < 0x1A || fg.CraftType > 0x1D) && (fg.CraftType < 0x37 || fg.CraftType > 0x3B) && (fg.CraftType < 0x46 || fg.CraftType > 0x4A) && (fg.CraftType < 0x50 || fg.CraftType > 0x59) && fg.CraftType != 0x86 && (fg.CraftType < 0x98 || fg.CraftType > 0x9B) && fg.CraftType != 0xB7)
                {
					...
					for (int o = 0; o < 16; o++)
                    {
						if (fg.Orders[o / 4, o % 4].Command != 0)
                        {
							hasOrder = true;
							break;
                        }
				...
				for (int o = 0; o < 16; o++)
					if (isBadTrigger(fg.Orders[o / 4, o % 4].SkipTriggers[0], fg.Orders[o / 4, o % 4].SkipTriggers[1], fg.Orders[o / 4, o % 4].SkipT1OrT2))
		...
		bool isBadTrigger(BaseTrigger trig1, BaseTrigger trig2, bool andOr) =>
			...
			((trig1.Condition == 0 || trig2.Condition == 0) && andOr) ||
